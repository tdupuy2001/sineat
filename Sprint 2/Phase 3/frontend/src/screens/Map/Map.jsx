...
function Map() {
  ...
  const [mapCenter, setMapCenter] = useState([
    43.92517082408836, 2.147408244346074,
  ]);
  const mapRef = useRef();
  ...
  useEffect(() => {
    if (selectedType) {
      ...
      service
        .getEtablissementsByType(selectedType)
        .then((response) => {
          if (response.data) {
        ...
        .catch((error) => {
          console.error(
            "Failed to fetch etablissements for type:",
            selectedType,
            error
          );
        });
  ...
  useEffect(() => {
    ...
    if (etablissements.length > 0) {
      const fetchDetails = async () => {
        const updatedEtablissements = await Promise.all(
          etablissements.map(async (etab) => {
            try {
              ...
              const coordResponse = await service.getEtablissementCoord(
                address
              );
              ...
              console.error(
                "Failed to fetch details for establishment:",
                etab.nom,
                error
              );
  ...
  const handleRegimeChange = async (event) => {
    ...
    try {
      ...
      console.error(
        "Failed to fetch etablissements for regime:",
        regimeId,
        error
      );
    }
          ...
          <div className="list-cardrestau">
            {etablissementsNote.map((etablissement) => (
              <RestauCard
                ...
                setSelectedRestauId={setSelectedRestauId}
                selectedRestauId={selectedRestauId}
        ...
        <div className="map-section">
          <div className="filter-bar">
            ...
            <FormControl style={{ flex: 1, minWidth: 120, marginRight: "5px" }}>
              <InputLabel id="regime-select-label" size="small">
                Regime
              </InputLabel>
              <Select
          ...
          <div className="map-cadre">
            ...
            >
              ...
              />
              {etablissementsNote.map(
                (etab) =>
                  etab.coord && (
                    <Marker
                      key={etab.id_etablissement}
                      position={[
                        etab.coord.geometry.coordinates[1],
                        etab.coord.geometry.coordinates[0],
                      ]}
                      icon={
                        selectedRestauId === etab.id_etablissement
                          ? redIcon
                          : DefaultIcon
                      }
                    >
