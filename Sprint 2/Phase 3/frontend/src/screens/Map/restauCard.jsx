...
export default function RestauCard({
  ...
  const toggleDetails = () => {
    if (data.coord) {
      const coordinates = [
        data.coord.geometry.coordinates[1],
        data.coord.geometry.coordinates[0] - 0.005,
      ];
    ...
    <>
        ...
        <Card sx={{ display: "flex", margin: 1 }}>
          ...
          <Box sx={{ display: "flex", flexDirection: "column" }}>
            <CardContent sx={{ flex: "1 0 auto" }}>
              ...
              >
                {data.notes[0]?.global_average != null
      ...
      {showDetails && (
        ...
        >
          ...
          <Typography variant="subtitle1" color="text.secondary" sx={{}}>
            ...
            sequi odio iure iusto quos voluptas dolore,
            {data.description}
          </Typography>
          {data.notes[0].notes.map((note, index) => (
            ...
            >
              ...
              <Rating
                name={`note-type-${index}`}
                value={note.average_grade}
                precision={0.1}
                readOnly
              />
            </div>
          ...
          {isLoggedIn && (
            ...
            >
              <StarIcon />{" "}
              <span style={{ fontSize: "15px" }}> Notez ce restaurant</span>
            </IconButton>
          )}
        </Box>
      )}
      ...
      <Dialog open={ratingDialogOpen} onClose={handleRatingDialogClose}>
        ...
        <DialogActions>
          ...
          <Button
            onClick={() => submitRating(/* Pass the rating value here */)}
          >
            Submit
          </Button>
        </DialogActions>
